...
class GPUModelRunner(
    ...
    @staticmethod
    def _dflash_obs1_diag_enabled() -> bool:
        return os.environ.get("DFLASH_OBS1_DIAG", "").strip().lower() in {
            "1",
            "true",
            "yes",
        }
    ...
    def _attach_dflash_verifier_state_probe(
        ...
        *,
        ...
        if self._dflash_obs1_diag_enabled():
            return
    ...
    def _attach_dflash_verifier_forward_probe(
        ...
        *,
        ...
        for req_idx, qlen in enumerate(query_lens):
            ...
            obs1_diag = self._dflash_obs1_diag_enabled()
            if not obs1_diag:
            ...
            if obs1_diag:
                self._attach_dflash_obs1_metrics(
                    bundle=bundle,
                    req_idx=req_idx,
                    req_logits=req_logits,
                    req_greedy=req_greedy,
    ...
    def _attach_dflash_obs1_metrics(
        self,
        *,
        bundle: dict[str, object],
        req_idx: int,
        req_logits: torch.Tensor,
        req_greedy: torch.Tensor,
        spec_decode_metadata: DFlashTreeSpecDecodeMetadata,
        start: int,
        qlen: int,
    ) -> None:
        """Compute Observation 1 scalars on GPU; do not dump full-vocab logits.

        Metrics follow the paper design: shared candidate row C_{d+1}, exact
        Target mass / greedy hit / regret via logsumexp, and full-vocab JS
        over sampled same-depth parent pairs.
        """
        drafter = getattr(self, "drafter", None)
        getter = getattr(drafter, "get_obs1_builder_state", None)
        builder = getter(req_idx) if callable(getter) else None
        if not builder:
            bundle["obs1_error"] = "missing_builder_state"
            return

        topk_tok = builder.get("builder_topk_tok")
        topk_lp = builder.get("builder_topk_lp")
        if not isinstance(topk_tok, torch.Tensor) or not isinstance(
            topk_lp, torch.Tensor
        ):
            bundle["obs1_error"] = "missing_builder_topk"
            return

        device = req_logits.device
        vocab = int(req_logits.shape[-1])
        depths = spec_decode_metadata.depths
        parents = spec_decode_metadata.parent_indices
        if depths is None or parents is None:
            bundle["obs1_error"] = "missing_tree_topology"
            return
        req_depths = depths[start : start + qlen].to(device=device, dtype=torch.long)
        req_parents = parents[start : start + qlen].to(device=device, dtype=torch.long)
        topk_tok = topk_tok.to(device=device, dtype=torch.long)
        topk_lp = topk_lp.to(device=device, dtype=torch.float32)
        draft_rows, tree_width = int(topk_tok.shape[0]), int(topk_tok.shape[1])

        logits_f = req_logits.float()
        logsumexp = torch.logsumexp(logits_f, dim=-1)
        log_probs = logits_f - logsumexp.unsqueeze(-1)
        greedy = req_greedy.to(device=device, dtype=torch.long)
        greedy_logp = log_probs.gather(1, greedy.unsqueeze(1)).squeeze(1)

        valid_parent = req_depths < draft_rows
        cand_row = req_depths.clamp(max=max(draft_rows - 1, 0))
        shared_cands = topk_tok[cand_row]
        shared_draft_lp = topk_lp[cand_row]
        cand_logp = log_probs.gather(1, shared_cands.clamp(0, vocab - 1))
        cand_mass = cand_logp.exp().sum(dim=-1)
        best_cand_logp = cand_logp.max(dim=-1).values
        regret = torch.clamp(greedy_logp - best_cand_logp, min=0.0)
        greedy_hit = (shared_cands == greedy.unsqueeze(1)).any(dim=-1).to(
            torch.float32
        )
        cand_mass = torch.where(valid_parent, cand_mass, torch.full_like(cand_mass, float("nan")))
        greedy_hit = torch.where(
            valid_parent, greedy_hit, torch.full_like(greedy_hit, float("nan"))
        )
        regret = torch.where(valid_parent, regret, torch.full_like(regret, float("nan")))
        cand_logp = torch.where(
            valid_parent.unsqueeze(1),
            cand_logp,
            torch.full_like(cand_logp, float("nan")),
        )

        seed_ranks = builder.get("seed_ranks")
        seed_edge_lp = builder.get("seed_edge_logprobs")
        if isinstance(seed_ranks, torch.Tensor):
            seed_ranks_cpu = seed_ranks.detach().cpu()
        else:
            seed_ranks_cpu = None
        if isinstance(seed_edge_lp, torch.Tensor):
            seed_edge_cpu = seed_edge_lp.detach().cpu()
        else:
            seed_edge_cpu = None

        parents_cpu = req_parents.detach().cpu().tolist()
        path_lp = [0.0] * qlen
        if seed_edge_cpu is not None and seed_edge_cpu.numel() >= qlen:
            edge = [float(v) for v in seed_edge_cpu[:qlen].tolist()]
            for node_idx in range(qlen):
                parent = int(parents_cpu[node_idx])
                if parent < 0:
                    path_lp[node_idx] = 0.0
                elif parent < node_idx:
                    path_lp[node_idx] = path_lp[parent] + edge[node_idx]

        max_pairs = int(os.environ.get("DFLASH_OBS1_MAX_PAIRS", "32"))
        max_pairs = max(1, max_pairs)
        depths_cpu = req_depths.detach().cpu().tolist()
        by_depth: dict[int, list[int]] = {}
        for node_idx, depth in enumerate(depths_cpu):
            by_depth.setdefault(int(depth), []).append(node_idx)

        tokens_cpu = bundle.get("tree_node_token_ids")
        token_list = (
            tokens_cpu.detach().cpu().tolist()
            if isinstance(tokens_cpu, torch.Tensor)
            else list(tokens_cpu or [])
        )

        def _path_tokens(node_idx: int) -> list[int]:
            path: list[int] = []
            cur = node_idx
            seen = 0
            while cur >= 0 and seen <= qlen:
                if cur < len(token_list):
                    path.append(int(token_list[cur]))
                parent = int(parents_cpu[cur]) if cur < len(parents_cpu) else -1
                if parent >= cur:
                    break
                cur = parent
                seen += 1
        ...
        def _lcp(left: list[int], right: list[int]) -> int:
            count = 0
        ...
        req_id = str(bundle.get("req_id", "unknown"))
        step = int(bundle.get("verify_step_index", -1))
        pair_left: list[int] = []
        pair_right: list[int] = []
        ...
        for depth, nodes in sorted(by_depth.items()):
            ...
            digest = hashlib.sha256(
                f"{req_id}:{step}:{depth}:obs1".encode()
            ).digest()
            rng = random.Random(int.from_bytes(digest[:8], "big"))
            rng.shuffle(candidates)
            for left, right in candidates[:max_pairs]:
                left_path = _path_tokens(left)
                right_path = _path_tokens(right)
                if left_path == right_path:
                    continue
                lcp = _lcp(left_path, right_path)
                ...
                pair_meta.append(
                    {
                        ...
                        "lcp_tokens": int(lcp),
                        "branch_distance": max(0, int(depth) + 1 - int(lcp)),
                    }
                )

        if pair_left:
            left_t = torch.tensor(pair_left, dtype=torch.long, device=device)
            right_t = torch.tensor(pair_right, dtype=torch.long, device=device)
            logp_u = log_probs[left_t]
            logp_v = log_probs[right_t]
            p = logp_u.exp()
            q = logp_v.exp()
            mixture = 0.5 * (p + q)
            log_m = torch.log(mixture.clamp_min(1e-30))
            js = 0.5 * (
                (p * (logp_u - log_m)).sum(dim=-1)
                + (q * (logp_v - log_m)).sum(dim=-1)
            )
            overlap_vals: dict[int, torch.Tensor] = {}
            for k in (1, 5, 10):
                top_u = torch.topk(logp_u, k=min(k, vocab), dim=-1).indices
                top_v = torch.topk(logp_v, k=min(k, vocab), dim=-1).indices
                overlap = []
                for row in range(top_u.shape[0]):
                    set_u = set(int(t) for t in top_u[row].tolist())
                    set_v = set(int(t) for t in top_v[row].tolist())
                    overlap.append(len(set_u & set_v) / float(k))
                overlap_vals[k] = torch.tensor(overlap, dtype=torch.float32)
            js_cpu = js.detach().cpu().tolist()
            for i, meta in enumerate(pair_meta):
                meta["js"] = float(js_cpu[i])
                meta["overlap_1"] = float(overlap_vals[1][i])
                meta["overlap_5"] = float(overlap_vals[5][i])
                meta["overlap_10"] = float(overlap_vals[10][i])

        bundle["obs1"] = {
            "builder_topk_tok": topk_tok.detach().cpu(),
            "builder_topk_lp": topk_lp.detach().cpu(),
            "seed_ranks": seed_ranks_cpu,
            "seed_edge_logprobs": seed_edge_cpu,
            "seed_path_logprobs": path_lp,
            "propose_step": builder.get("propose_step"),
            "node_logsumexp": logsumexp.detach().cpu(),
            "node_mass": cand_mass.detach().cpu(),
            "node_greedy_hit": greedy_hit.detach().cpu(),
            "node_regret": regret.detach().cpu(),
            "node_candidate_target_logprobs": cand_logp.detach().cpu(),
            "valid_parent": valid_parent.detach().cpu(),
            "pairs": pair_meta,
        }
    ...
    def _compact_dflash_tree_hidden_states(
        ...
        for req_idx, path_gpu in enumerate(accept_paths_gpu):
            ...
            verify_bundles = getattr(self, "_dflash_runtime_verify_bundles", None)
            skip_hidden_dump = self._dflash_obs1_diag_enabled()
            if (
                verify_bundles
                and not skip_hidden_dump
                and "post_compact_hidden_states" not in verify_bundles[-1]
            ):
            ...
            if aux_hidden_states is not None:
                ...
                if (
                    verify_bundles
                    and not skip_hidden_dump
                    and "post_compact_target_hidden_states" not in verify_bundles[-1]
                ):
